  ...
  return (
    <Box bgcolor='#000' position="fixed" width="100%" paddingBottom="7px" boxShadow="0 0 10px gray" className='p-2'>
      ...
      <Modal open={isModalOpen1} onClose={handleCloseModal}>
        <Box sx={{ position: 'absolute', top: '20%', left: '60%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', borderRadius: '8px' }}>
          <h2 className='uppercase text-center font-medium'>Notifications</h2>
          <hr></hr>
          {notifications.map((notification, index) => (
            <div key={index}>
           <div className='flex jgap-3 m-3'>
...
{notifications1.map((notification, index) => (
           ...
           <p><span className='font-bold'>{notification.username}  </span>{notification.message}</p>

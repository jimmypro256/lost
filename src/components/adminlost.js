...
function AdminLost() {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/Lost/${editFormData._id}`, {
        method: 'PUT',
        headers: {
      ...
      <AdminSidebar/>
          ...
          <Box width={{ xs: '100%', sm: '85%' }} className="border border-slate-400 bg-white " height={{ sm: '75vh' }} >
                  ...
                  <div className=' flex gap-5'>
                    ...
                    <Link underline="hover" color="primary" to="/adminfound">
       ...
       {/* Description */}
                ...
                <hr></hr>
                 ...
                 {item.reward !== '' && (
          <Button variant='outlined' color='success' onClick={() => handleRewardButtonClick(item.reward)}>REWARD</Button>
